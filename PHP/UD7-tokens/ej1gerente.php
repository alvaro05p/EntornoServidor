...
<?php
    ...
    if (!isset($_GET['nombres'])) {
        if (!isset($_GET['numTrabajadores']) && isset($_GET['calcular'])) {
            // Mostrar el primer formulario para el n̼mero de trabajadores
?>    
            <form action="ej1gerente.php" method="GET">
...
<?php
                ...
                <select name="estadisticas[]" class="form-select" multiple required size="3">
                    <option value="maximo">Salario maximo</option>
                    <option value="minimo">Salario minimo</option>
                    <option value="medio">Salario medio</option>
                </select><br><br>

                <input type="submit" value="Calcular" class="btn btn-primary" name="calcular">
